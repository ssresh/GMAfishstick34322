var referencesList;
if (
  $("#questionDetailsPage1").length > 0 ||
  $("#sampleDetailsPage1").length > 0
) {
...
*/
    ...
    },
    {
      ...
      templates: {
        suggestion: function (data) {
          return '<div class="subsuc">' + data.value + "</div>";
        },
      },
    }
  );
  ...
  $("#sidebarForm ,#sidebarForm1, #firstViewForm, #secondLabelFrom").submit(
    function (e) {
      var formId = "";
      var form = $(this);
      ...
      var error1 = true;
      var error2 = !emailValidate(
        "inputEmail" + formId,
        $("#inputEmail" + formId).val()
      );
      var error3 = true;
      if ($.trim($("#subject" + formId).val()) == "") {
        $("#subject" + formId).addClass("animated shake shake-error");
        setTimeout(function () {
          $("#subject" + formId).removeClass("animated shake");
        }, 1000);
      } else {
      ...
      if ($("#deadlineDate" + formId).val() == "") {
        ...
        setTimeout(function () {
          $("#deadlineDate" + formId).removeClass("animated shake");
        }, 1000);
      } else {
      ...
      if ($("#orderFormAttachmentModal").is(":visible")) {
        ...
        if (requirement == "" && fileUploadCount == 0) {
          ...
          setTimeout(function () {
            $("#requirements").removeClass("animated shake");
            $("#uploadFiles3").removeClass("animated shake");
          }, 1000);
          ...
          $("#requirementFileHtml" + formId).html(
            $("#requirementSection").html()
          );
  ...
  $("#firstOrderform").submit(function (e) {
    var error = emailValidate(
      "firstOrderformEmail",
      $("#firstOrderformEmail").val()
    );
    if (error != true) {
      e.preventDefault();
    }
  ...
  $(".datetimeSelector1, .datetimeSelector, .datetimeSelector2").click(
    function () {
  ...
  /*
	...
	*/
  if (
    $("#expertSubject").length ||
    $("#expertListData").length ||
    $("#expertcarousel").length ||
    $("#expertListDataAu").length
  ) {
  ...
  if ($("#secondLabelFrom").is(":visible")) {
    $(
      '#sidebarForm input[name ="email"], #sidebarForm input[name ="subject"], #sidebarForm input[name ="deadline"]'
    ).removeAttr("tabindex");
...
$(window).scroll(function () {
  if ($(window).scrollTop() > 300) {
    $("#omenu").addClass("omenuMove");
  } else {
    $("#omenu").removeClass("omenuMove");
  }
});
...
function showPreview(fileUrl, fileName) {
  ...
  $("#showFileIframeModalBody").html(
    '<iframe style="width: 100%;height: 500px;" src="https://docs.google.com/gview?url=' +
      fileUrl +
...
function showPrice(currency) {
  ...
  $("html, body").animate(
    { scrollTop: $(".price-details-row").offset().top },
    2e3
  );
  return;
  $(".priceTable").addClass("hide");
  ...
  if (emailValidate("frgtPass", email)) {
    ...
    $.ajax({
      ...
      success: function (status) {
        ...
        if (status == "success") {
          $("#responseMsg").text(
            "We just sent you an email with your password. The e-mail should show up within few seconds. If it doesn't, check your spam."
          );
          ...
          $("#responseMsg").text(
            "It seems like you have entered a wrong/invalid email id. Please enter a correct one."
          );
...
function submitSubscriber() {
  ...
  if ("" == email || !validateEmail(email) || "" == source)
    return (
...
function validatePlagiarismForm() {
  ...
  var count = 0;
  if (
    "" == plagiarismEmailValue ||
    !validateEmail(plagiarismEmailValue) ||
    plagiarismEmailValue.length > 100
  ) {
    plagiarismEmail.removeClass("inputSuccess");
    plagiarismEmail.addClass("animated shake");
    plagiarismEmail.addClass("shake-error");
    setTimeout(function () {
      plagiarismEmail.removeClass("animated shake");
    }, 1000);
  ...
  if ("" == plagiarismDeadlineTimeValue) {
    ...
    setTimeout(function () {
      plagiarismDeadlineTime.removeClass("animated shake");
    }, 1000);
  ...
  if (plagiarismPhoneValue.length < 6 || plagiarismPhoneValue.length > 13) {
    ...
    setTimeout(function () {
      plagiarismPhone.removeClass("animated shake");
    }, 1000);
  ...
  if (count == 0) {
    ...
    setTimeout(function () {
      $("#fileUploadType").removeClass("animated shake");
    }, 1000);
...
function validateAffiliateForm(formNo, no) {
  ...
  if ("" == name) {
    ...
    setTimeout(function () {
      $("#name" + no).removeClass("animated shake");
    }, 1000);
  ...
  if ("" == email || !validateEmail(email) || email.length > 100) {
    ...
    setTimeout(function () {
      $("#email" + no).removeClass("animated shake");
    }, 1000);
    $("#email" + no).focus();
    error2 = true;
  } else {
  ...
  if ("" == phone || phone.length < 9 || phone.length > 13) {
    ...
    setTimeout(function () {
      $("#phone" + no).removeClass("animated shake");
    }, 1000);
...
function ratingStar(a) {
  $("#sendReview .modal-content .form-group .star-border .fa").css(
    "color",
    "#e3e3e3"
  );
...
$("#sendReview").submit(function (e) {
  ...
  if ($.trim($("#orderId").val()) == "" || $("#orderId").val().length < 15) {
    ...
    $("#orderId").addClass("shake-error");
    setTimeout(function () {
      $("#orderId").removeClass("animated shake");
    }, 1000);
    error1 = false;
  } else {
  ...
  if ($.trim($("#reviewTitle").val()) == "") {
    ...
    $("#reviewTitle").addClass("shake-error");
    setTimeout(function () {
      $("#reviewTitle").removeClass("animated shake");
    }, 1000);
    error3 = false;
  } else {
  ...
  if ($.trim($("#reviewComment").val()) == "") {
    ...
    $("#reviewComment").addClass("shake-error");
    setTimeout(function () {
      $("#reviewComment").removeClass("animated shake");
    }, 1000);
    error4 = false;
  } else {
  ...
  if ($.trim($("#ratingBox").val()) == "") {
    ...
    $(".star-border").addClass("shake-error");
    setTimeout(function () {
      $(".star-border").removeClass("animated shake");
    }, 1000);
    error5 = false;
  } else {
  ...
  if (
    ...
    $.smkAlert({
      text: "Thanks for submitting the review. It is under moderation, will be published once approved.",
      type: "success",
    });
    return false;
    $("orderId").val("");
...
if (typeof filledSubject !== "undefined") {
  if (filledSubject != "") {
    $.ajax({
      ...
      success: function (e) {
        e
          ? ($("#pages").show(), $("#reference").show())
          : ($("#pages").hide(), $("#reference").hide());
      },
      async: !0,
    });
  }
...
function applyCallbackRequest(e) {
  ...
  if (!error) {
    ...
    $.ajax({
      ...
      dataType: "json",
      success: function (e) {
        $("#submitCallbackRequest").button("reset"),
          $.smkAlert({
            text: e.msg,
            type: e.status,
          }),
...
function addAssignmentFeedbackData() {
  ...
  if ("" == userRespond) {
    ...
    setTimeout(function () {
      $("#userRespond").removeClass("animated shake");
    }, 1000);
  ...
  if (userRespond == "resultAwaited" && "" == nextFeedbackDate) {
    ...
    setTimeout(function () {
      $("#nextFeedbackDate").removeClass("animated shake");
    }, 1000);
  ...
  if (userRespond == "responseReceived" && "" == tutorScore) {
    ...
    setTimeout(function () {
      $("#tutorScore").removeClass("animated shake");
    }, 1000);
  ...
  if (userRespond == "responseReceived" && "" == supportScore) {
    ...
    setTimeout(function () {
      $("#supportScore").removeClass("animated shake");
    }, 1000);
  ...
  if (userRespond == "responseReceived" && "" == assignmentScore) {
    ...
    setTimeout(function () {
      $("#assignmentScore").removeClass("animated shake");
    }, 1000);
  ...
  if ("" == feedbackComment) {
    ...
    setTimeout(function () {
      $("#feedbackComment").removeClass("animated shake");
    }, 1000);
...
function requestQuestionBankSolution(
  questionBankId,
  assignmentId,
  solutionType,
  button
  ...
  if (email.trim() == "" || !validateEmail(email) || email.length > 100) {
    ...
    setTimeout(function () {
      $("input[name=e").removeClass("animated shake");
    }, 1000);
    $("input[name=e").focus();
    return false;
  }
  if (
    ...
    $.ajax({
      ...
      data: $.param({
        ...
        email: email,
        checkFakeUserbv451: checkFakeUserbv451,
        assignmentId: assignmentId,
        solutionType: solutionType,
        descountedPrice: descountedPrice,
        currency: currency,
        questionBankOrderId: id,
      }),
      dataType: "json",
      success: function (data) {
        if (data.responseMsg.msg == "success") {
          window.location.assign(
            questionBankUrl +
              ...
              title +
              "&aid=" +
              data.responseMsg.assignmentId +
              "&stype=" +
              solutionType +
              "&userId=" +
              data.responseMsg.userId
          );
    ...
    alert(
      "Your request can not be completed at the moment please try after some time"
    );
  }
}
...
function sticky_relocate() {
  ...
  if ($sticky.offset()) {
    ...
    var stickyRight =
      $(window).width() - ($sticky.offset().left + $sticky.outerWidth());
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if (data.status == "success") {
          ...
          $("#offerSuccessMsg" + no).html(
            "We have sent the coupon codes to you at your given email address. Please check for the same..."
          );
          $("#offerSuccessMsg" + no).css("color", "green");
          $("#offerForm" + no)[0].reset();
        } else {
          $("#offerSuccessMsg" + no).html(
            "There are some issues, Please try again"
          );
...
function goToByReviews(reviewType) {
  $("html,body").animate(
    { scrollTop: $("#homeTestimonialSection").offset().top - 30 },
    "slow"
  );
  $("." + reviewType).click();
}
...
function submitQuestionAnswerByUser() {
  ...
  if ("" == userEmail || !validateEmail(userEmail) || userEmail.length > 100) {
    ...
    setTimeout(function () {
      $("#userEmail").removeClass("animated shake");
    }, 1000);
    $("#userEmail").focus();
    error = true;
  } else {
  ...
  if ("" == userQuestionAnswer) {
    ...
    setTimeout(function () {
      $("#userQuestionAnswer").removeClass("animated shake");
    }, 1000);
...
function checkAlreadyAnswerSubmitByUser() {
  var questionBankId = $("#questionBankId").val().trim();
  var storageQuestionBankId = localStorage.getItem(
    "submitAnswer_" + questionBankId
  );
...
function sendLoginOtp(actionType) {
  ...
  if (contactNumber.length < 8) {
    ...
    setTimeout(function () {
      $("#contactNumber").removeClass("animated shake");
    }, 1000);
    $("#contactNumber").focus();
    return false;
  } else {
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data.response == "success") {
          ...
          $("#otpResponse").html(
            "OTP could not be sent. Please contact to support team over chat"
          );
...
function sendOfferOnWhatsapp(no) {
  ...
  if ("" == whatsappNumber) {
    ...
    setTimeout(function () {
      $("#offerWhatsappNumber" + no).removeClass("animated shake");
    }, 1000);
...
function setSelectedPhoneCode(phoneCode, countryCode, no) {
  $("#selectedPhoneCode" + no).html(
    "<img src='" +
      imageUrl +
      "/flags/" +
      countryCode.toLowerCase() +
      ".png' style='display:inline-block'></span><span id='phoneCodePick'> + " +
      phoneCode +
      "</span>"
  );
